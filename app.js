...
class App extends Component {
  ...
  componentDidMount = () => {
    this.IntervalId = setInterval(this.increaseTheTimer, 1000)
  }
  ...
  onClickThumbnailFruitItem = eachitemDetails => {
    ...
    if (activeImage.id === eachitemDetails.id) {
      const randomIndex = Math.floor(Math.random() * imagesList.length)
      this.setState(prevstate => ({
        activeImage: imagesList[randomIndex],
  ...
  gameScoreCard = () => {
    ...
    return (
      <div>
        <div className="scorecard-background">
          <img
  ...
  render() {
    const {activetabId, timeElapsedInSeconds, activeImage, score, isGameOver} =
      this.state
    const filteredList = imagesList.filter(
      eachitem => eachitem.category === activetabId,
    )
    const scoreCard = this.gameScoreCard()
    return (
      <div>
          ...
          <div>
            {isGameOver ? (
              ...
              <>
                ...
                <ul className="fruitlist-container">
                  {filteredList.map(eachitem => (
                    <FruitItem
                      eachitemDetails={eachitem}
                      key={eachitem.thumbnailUrl}
                      onClickThumbnailFruitItem={this.onClickThumbnailFruitItem}
                    />
                  ))}
                </ul>
              </>
